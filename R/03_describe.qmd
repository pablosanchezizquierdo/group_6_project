...
```{r}
# We need to use another library called "maps":
library(maps)
...
world_map_plot <- data_clean |> 
  ...
  select(Country) |> 
  group_by(Country) |> 
  summarise(log10Count = log(n(), 10)) |>     # We need to do log(,10) since Canada has way many 
...
ggsave("../results/heat_map_of_bacteria_at_each_country.png", plot7, width = 10, height = 6, dpi = 300)
```
