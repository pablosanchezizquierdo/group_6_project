...
data_clean <- read_delim("../data/clean_data.tsv", delim = "\t", show_col_types = FALSE)
...
plot2 <- ggplot(data_clean, aes(x = `Compound Name`, fill = `Encoding Gene`)) +
...
temp_uncultured <- data_clean |> 
  mutate(Has_Uncultured = str_detect(`Strain ID/Microorganism`, "Uncultured"))
...
## Heat map of Organisms at each Continent
