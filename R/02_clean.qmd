...
We can see how the variables with data related to location and the "Protein ID" and the "UniProt ID" variable are the ones with the most number of NA values. For now, we can leave it like this. Eventually, we can create a unique identifier for each unique observation and then reassess the NA situation.
...
```{r}
unique_countries2 <- selected_columns |> 
  distinct(Country) |> 
  arrange(Country)
