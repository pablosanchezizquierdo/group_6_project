...
all_sheets <- read_delim("../data/all_sheets.tsv", delim = "\t")
...
nan_counts <- colSums(is.na(selected_columns))
...
unique_values_Continent <- selected_columns |> 
  select("Continent") |> 
  distinct() 
...
Changes to be made:
- "Mediterranean sea" & "Mediterranean Sea" should be part of the "Atlantic Ocean".
- Geographically, the "Middle East" is an ambiguous region in terms of continent. It encompasses 3 continents: Asia, Europe and Africa, so the continent should be decided by the country variable.
- "Central America" will be considered part of "North America"
- "Northern Atlantic Ocean" is part of the "Atlantic Ocean"
- "South Pacific Ocean" is part of the "Pacific Ocean"
...
selected_columns <- selected_columns %>%
  ...
  mutate(Continent = ifelse(ToContinent == "South America, Europe", 
                            NA, 
                            ifelse(!is.na(ToContinent),
                                   ToContinent, 
                                   Continent))) |> 
  select(-ToContinent)
