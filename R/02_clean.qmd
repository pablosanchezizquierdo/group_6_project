...
How we'll deal with this: - We'll first extract a tibble with the unique gene names in the data set. - We'll work on this tibble. - Then, we'll substitute the names in the data set with the corresponding unique names in the tibble.
...
# We mutate the names:
selected_columns$`Encoding Gene` <- selected_columns |> 
  select(`Encoding Gene`) |> 
