...
```{r}
data_reduced <- data_reduced |> 
  rename(Continent = `Continent where study sampling was done (Collections)`)
```
...
```
The names that we should change are:
- 'Hexachlorobenzene (HCB)' and 'Hexachlorobenzene' should be the same category.
- 'Pentachlorobenzene' and 'Pentachlorbenzene' should be the same category too.
```{r}
# Sample conditions list
...
```{r}
data_reduced <- data_reduced |>
  rename("Compound_Name" = "Compound Name", "Enzyme_Name" = "Enzyme Name", "Encoding_Gene" = "Encoding Gene", "KEGG_Orthology" = "KEGG Orthology", "Strain_ID/Microorganism" = "Strain ID/Microorganism", "Protein_ID" = "Protein ID")
```
